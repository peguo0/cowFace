...
if [ $# -lt 3 ]; then
    echo
    echo " Usage: $0 <test img dir> <model prefix> <outFilePath> <epoch> [cropCoords]"
    echo
...
cmd="python3 $codeDir/genEmbedding.py $imgDir $modelPrefix $epoch $outPath --gpu=0 --crop=$crop"
echo "> $cmd"
eval "$cmd"
